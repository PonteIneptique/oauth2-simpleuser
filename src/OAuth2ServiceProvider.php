 ...
 *
 *
 *  Modification made so it would mount properly on given path
 * @author Thibault Clerice <leponteineptique@gmail.com>
 */
...
/*
 *  Disabled as it is not used in our own project
 *
        $controllers->match('/api/v1.0/oauth2/debug', 'authbucket_oauth2.oauth2_controller:debugAction')
            ->bind('api_oauth2_debug')
            ->method('GET|POST');

        $controllers->get('/api/v1.0/oauth2/cron', 'authbucket_oauth2.oauth2_controller:cronAction')
            ->bind('api_oauth2_cron');

        foreach (array('authorize', 'client', 'scope') as $type) {
            $controllers->post('/api/v1.0/'.$type.'.{_format}', 'authbucket_oauth2.'.$type.'_controller:createAction')
                ->bind('api_'.$type.'_create')
                ->assert('_format', 'json|xml');

            $controllers->get('/api/v1.0/'.$type.'/{id}.{_format}', 'authbucket_oauth2.'.$type.'_controller:readAction')
                ->bind('api_'.$type.'_read')
                ->assert('_format', 'json|xml');

            $controllers->put('/api/v1.0/'.$type.'/{id}.{_format}', 'authbucket_oauth2.'.$type.'_controller:updateAction')
                ->bind('api_'.$type.'_update')
                ->assert('_format', 'json|xml');

            $controllers->delete('/api/v1.0/'.$type.'/{id}.{_format}', 'authbucket_oauth2.'.$type.'_controller:deleteAction')
                ->bind('api_'.$type.'_delete')
                ->assert('_format', 'json|xml');

            $controllers->get('/api/v1.0/'.$type.'.{_format}', 'authbucket_oauth2.'.$type.'_controller:listAction')
